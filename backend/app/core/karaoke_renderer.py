...
"""
        ...
        for i, lyric in enumerate(lyrics):
            if "start" in lyric and "end" in lyric:
                ...
                k_time = int(duration * 100)  # 转换为厘秒
                karaoke_text = f"{{\\k{k_time}}}{text}"
                ...
                # 整句卡拉OK效果
                karaoke_text = self._create_sentence_karaoke(
                    text, lyric["time"],
                    lyrics[i + 1]["time"] if i < len(lyrics) - 1 and "time" in lyrics[i + 1] else lyric["time"] + 5
                )
    ...
    def update_lyrics_timing(self, ass_path: str, time_offset: float):
        ...
        Args:
            ass_path: ASS字幕文件路径
            time_offset: 时间偏移量（秒），正数表示提前，负数表示延后
        """
        with open(ass_path, "r", encoding="utf-8") as f:
            content = f.read()
